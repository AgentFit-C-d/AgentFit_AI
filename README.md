...
Spec, plan, tasks and measured limitations: [Solar analysis](specs/ai-developer/04-analysis-provider/README.md). Latest Solar tuning experiment (profile-v16): 66 unit tests, 24/24 existing synthetic runs, 2/2 new synthetic cases, and 2/4 real-document runs passed. The quality gate remains unmet; the experimental branch is not ready for adoption. See [current validation](specs/ai-developer/04-analysis-provider/solar-tuning/validation.md). HTTP endpoints, PDF extraction and Spring integration remain future work.
