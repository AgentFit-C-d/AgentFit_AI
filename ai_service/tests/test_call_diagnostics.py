...
class CallDiagnosticsTests(unittest.TestCase):
    ...
    def test_second_call_timeout_identifies_stage_without_retry(self):
        ...
        with self.assertRaises(AnalysisError) as caught:
            SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport).analyze("Alpha registration", "doc")
    ...
    def test_repaired_success_never_persists_raw(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            r = SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport, diagnostics_store=store).analyze("Alpha registration", "doc")
    ...
    def test_final_failure_stores_only_failed_responses(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            with self.assertRaises(AnalysisError):
                SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport, diagnostics_store=store).analyze("Alpha registration", "doc")
            saved = json.loads(next(Path(folder).glob("*.json")).read_text())
    ...
    def test_sensitive_response_is_not_saved(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            with self.assertRaises(AnalysisError):
                SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport,
    ...
    def test_incomplete_response_retains_usage_metadata(self):
        ...
        with self.assertRaises(AnalysisError) as caught:
            SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport).analyze("Alpha", "doc")
    ...
    def test_store_failure_preserves_original_analysis_error(self):
        ...
        store.write.side_effect = OSError("private path")
        with self.assertRaises(AnalysisError) as caught:
            SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=Mock(side_effect=TimeoutError()),
    ...
    def test_nested_escaped_key_and_invalid_json_are_omitted(self):
        ...
        for raw in (json.dumps(payload).encode(), b"not-json"):
            with self.subTest(raw=raw), tempfile.TemporaryDirectory() as folder:
                with self.assertRaises(AnalysisError):
                    SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=Mock(return_value=raw),
                        diagnostics_store=LocalDiagnosticsStore(Path(folder))).analyze("Alpha", "doc")
                saved = json.loads(next(Path(folder).glob("*.json")).read_text())
                self.assertEqual(saved["failed_responses"], [])
    ...
    def test_preflight_failure_has_no_provider_calls_or_storage(self):
        ...
        transport = Mock()
        with self.assertRaises(AnalysisError) as caught:
            SolarAnalyzer("synthetic-key", evidence_contract=False, semantic_review=False, transport=transport, diagnostics_store=store).analyze("", "doc")
